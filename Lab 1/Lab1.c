...
// Date:         1-6-2012
// Authors:      Dr. Don Cox
// Modified by:  Ryan Courreges
//
// Description:  Software code for Lab 1 assignment for ECE 372 Spring 2012. Sample code toggles a user 
//               specified LED on the Microchip 16-bit 28-pin starter board. The user specified LED is received
//               using the PIC's UART.
//
// Requirements: This software code requires the MPLAB C30Compiler (MPLAB C Compiler for PIC24 MCUs).
//
// Credits:  Software code based upon sample code provided with Microchip 16-bit 28-pin development board.
//
// ******************************************************************************************* 
//
...
#define DELAY_AMOUNT    228                   // Delay for button debouncing
...
int main(void)
{
	// Variable for character recived by UART.
	int receivedChar;
	// for loop control index
	int i;
	...
	//     RIDLE             = 0     (Reciver is active)U1STA  = 0x0440;  // Reset status register and enable TX & RX
	...
	while(1)
	{
		...
		if( PORTBbits.RB5 == 0 ) {
			...
			// Change blink rate using Timer1's period register
			TMR1 = 0;
			PR1 = 7199;

			// button press delay for debounce
			for(i=0; i<DELAY_AMOUNT; i++) i=i;

			// wait for button release
			while( PORTBbits.RB5 == 0 );

			// Reset timer coutner and blink rate to standard
			TMR1 = 0;
			PR1 = 14399;

			// button release delay for debounce
			for(i=0; i<DELAY_AMOUNT; i++) i=i;
		}
		
		// Use the UART RX interrupt flag to wait until we receive a character.
		if(IFS0bits.U1RXIF == 1) {
			...
			if( receivedChar <= '7' && receivedChar >= '4' ) {	
				...
				// Clear all LED output to make sure only one LED is blinking at a time
				LATBbits.LATB15 = 1;
				LATBbits.LATB14 = 1;
				LATBbits.LATB13 = 1;
				LATBbits.LATB12 = 1;		
				...
				// the character '0'.
				ledToToggle = receivedChar - '0';// Print a confirmation message.
